...
from models.misc_models import init_weights
...
class SpatialCorrelativeLoss(nn.Module):
    ...
    def create_conv(self, feat, layer):
        ...
        self.init_net(conv, self.init_type, self.init_gain, self.gpu_ids)
...
class VGG16(nn.Module):
    def __init__(self):
        ...
        for x in range(2):
            if x == 0:
                tmp = nn.Conv2d(1, 64, 3, 1, 1)
                self.relu1_1.add_module(str(x), tmp)
            else:
