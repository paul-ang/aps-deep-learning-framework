...
class NLayerDiscriminator(nn.Module):
    ...
    def forward(self, input, layers: list = []):
        """Standard forward."""
        if len(layers) > 0:
            feat = input
            feats = []
...
class MsDiscriminator(nn.Module):
    def __init__(self, n_scales=3, input_nc=1):
        super().__init__()
        self.disc = nn.ModuleList()

        for _ in range(n_scales):
            self.disc.append(define_D(input_nc=input_nc, ndf=64, netD='basic',
    ...
    def forward(self, x):
        output = []
        for i, d in enumerate(self.disc):
            factor = 1 / (2 ** i)
            input = torch.nn.functional.interpolate(x, scale_factor=factor, align_corners=False, mode='bilinear')
